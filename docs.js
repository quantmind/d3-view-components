...
var formData = {
    model: {
        formData: '',
        $formFieldChange: function $formFieldChange(field) {
            var data = field.form.$inputData(),
                mkd = ["#### Form inputs", "```json", JSON.stringify(data, null, '    '), "```"];
            this.formData = mkd.join('\n');
        }
    },

    render: function render(props, attrs, ol) {
...
function componentsPlugin(d3) {
    ...
    return function (vm) {
        ...
        vm.use(d3.viewModal);
        //
        d3.viewRouter(vm, { routes: routes });
        //
...
window.d3.require('d3-view-components', 'd3-view').then(function (d3) {
    ...
    }).use(componentsPlugin(d3)).use(sitePlugin).use(d3.viewForms).use(d3.viewBootstrapForms).mount('body');
});
